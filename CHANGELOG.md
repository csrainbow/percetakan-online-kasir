...
- **Bukti bayar dibuat privat**: folder `/uploads/proofs/` diblokir untuk publik di `router.php` (sebelumnya file screenshot transfer bisa diakses siapa pun via URL langsung). Admin melihat bukti lewat endpoint baru `admin/proof-stream.php?f=…` (wajib login admin, anti path traversal, MIME sesuai, `no-store`) — `admin/order-detail.php` diperbarui memakainya.
- **Kode unik pembayaran dihapus**: nominal unik (total + kode 3 digit) tidak lagi dihasilkan/display untuk order baru, tidak lagi dipakai di pencocokan otomatis (`pm_match_hits` kini hanya mencocokkan **total persis** untuk transfer/QRIS), dan semua teks instruksi "nominal unik/kode unik" dibersihkan dari cek-pesanan, order-success, customer-service, term of service, sync-payment, dan pengaturan QRIS. Form upload bukti (`payment/confirm.php`) tidak lagi menganjurkan bayar nominal + kode; nominal maks = sisa tagihan.
- **Sisa tagihan dipastikan non-negatif**: setiap perhitungan `total − sudah dibayar` di-clamp `max(0, …)` dan persen terbayar `min(100, …)` di admin order-detail/pesanan, customer order-detail, cek-pesanan, pay-point, invoice, dashboard, finish/notification — order dengan kelebihan bayar lama (mis. `INV/20260910/AB8848` tercatat 45.275 utk total 45.000) tampil LUNAS dengan Sisa Rp 0, tidak lagi minus.
